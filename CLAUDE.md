...
## Architecture
- `plugin/scripts/sdlc/` stdlib-only package. `cli.py` dispatches `<stage> <action> [arg]` and returns one JSON dict (`ok`, `reason`, `next`, ...). `stages.py` owns the new/check/accept lifecycle shared by intent.md, spec.md, plan.md. `build.py` TDD log + plan sync + fix lock. `testing.py` feedback loop + review.md. `deploy.py` tiers, rollback rehearsal, PR body. `maintain.py` Western Electric bands that write the next intent.md. `checkpoint.py` the stage-boundary commit of generated artifacts; `cli.BOUNDARIES` names the boundaries and `cli.main` commits the verdict's `path` after a success (tests run with `SDLC_CHECKPOINT=off` unless they take the `checkpoint_on` fixture, listed before any `accepted_*` fixture). `hooks.py` PreToolUse/PostToolUse/SessionStart guardrails. `evals.py` continuous evals. `knowledge.py` Graphify bootstrap plus the OKF v0.2 bundle under `sdlc/knowledge/` (bootstrap/status/refresh/check/publish/unhook; Graphify and uv are subprocesses, tests stub them with the `knowledge` fixture). `workflows.py` the stage Workflow catalog (`plugin/workflows/*.js`, invoked as `sdlc:<name>`) and the session-start merge of `[workflows.env]` into `.claude/settings.local.json`; tests run with `SDLC_WORKFLOWS=off` unless they drop it. `docs.py` Archify stage documents (render/check/open plus the `archify` bootstrap step; Node and Archify are subprocesses, tests stub them with the `docs_tools` fixture). `packs.py` graph-selected Repomix context packs under `graphify-out/packs/` (`knowledge pack <stage>`, and `require`, the plan-accept and test-review gate; Repomix, npm and Bandit are subprocesses, tests stub them with the `packs` fixture and otherwise run with `SDLC_PACKS=off`).
...
- `plugin/workflows/*.js` Workflow scripts: `export const meta` is written as JSON (Python reads it), phase titles must match, no `Date.now`/`Math.random`; read-only agents, the command writes the artifact.
...
- Context packs: `packs.require` gates `plan accept` and `test review`; a test that takes both `accepted_*` and `packs` lists `accepted_*` first, or the accept is refused for want of a pack. Pack test repos exclude the sandbox (`bin/`, `home/`, `claude/`) in `.git/info/exclude`, or `git add -A` commits it and the graph looks stale.
