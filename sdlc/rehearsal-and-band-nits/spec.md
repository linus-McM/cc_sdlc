...
3. When `git worktree remove` exits non-zero after the rollback ran, `rehearse` records the rollback result in deploy.json with a `leftover` note (path, git's stderr, `git worktree prune` hint), then fails with that note as the reason. A failed rollback still owns the verdict when both fail.
