...
Gates: `plan accept` needs the plan pack at HEAD; `test review` needs the test pack at HEAD accounting for every changed text file, renames by their new path: packed, or listed in the manifest's exclusions (a symlink or git-ignored file); `missing` lists the rest, and a changed file matching a secret rule refuses the review. An empty selection writes an empty pack without running Repomix. Repomix's default ignore patterns are off in the plugin config, so a requested lock file is packed rather than silently dropped. With the layer on and Repomix absent both are refused with `npm i -g repomix`; design, build and maintain packs are advisory. The `repomix` bootstrap step (after `graph`, optional) installs with `npm i -g repomix` and, on an explicit `sdlc knowledge bootstrap` or `plan new`, updates with `npm update -g repomix`; session start only checks. Off switches: the knowledge layer's, and `SDLC_PACKS=off` (both gates then report `skipped`).
