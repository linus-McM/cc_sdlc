...
Plugin settings cannot set environment variables, so the session-start hook does it: `sdlc workflows env` merges `[workflows.env]` into `.claude/settings.local.json` (never overwriting a value already there; it reports those as `kept`) and appends `export` lines to `CLAUDE_ENV_FILE` for the current session's shell. The first session after install writes the file; the Workflow tool picks it up from the next session. `sdlc workflows list` reports the catalog and whether the layer is on. Off switches: `[workflows] enabled = false`, `[workflows] auto_env = false`, `SDLC_WORKFLOWS=off`; `disableWorkflows` in Claude Code settings still wins.
